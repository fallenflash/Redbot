    ...
    if (users.length !== 0 && timeDiff) {
        ...
        client.pool.query(current, [users.map(u => u.id).join(',')], function(err, res) {
            ...
            if (res) {
                ...
                res.forEach(result => {
                    const u = result.user;
                    const b = moment(result.ending).format('YYYY-MM-DD HH:mm:ss');
                    const e = moment(b).add(timeDiff, 'seconds').format('YYYY-MM-DD HH:mm:ss');
                    values.push([u, message.author.username, b, e]);
                });
                client.pool.query(newSub, [values], function(error, result) {
                    if (error) {
                    ...
                    if (result) {
                        ...
                        ids.forEach(id => {
                            client.addRole(id, client.config.server.subscriberRole)
                                .then(re => {
                                    client.logger.log(`added ${re.role} to ${re.member}`);
                                })
                                .catch(err => {
